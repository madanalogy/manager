...
import dev.madanalogy.manager.config.KafkaProperties;
...
class ManagerKafkaResourceIT {
    ...
    void setup() {
        ...
        ManagerKafkaResource kafkaResource = new ManagerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/manager-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/manager-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
